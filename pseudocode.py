...
from torch.nn import Parameter as Parameters
from netCDF4 import Dataset

model_path=None

dataset_length = 6 #hours
...
from torch.nn import LayerNorm, Dropout as DropOut, GELU as GeLU, Softmax as SoftMax
...
def Pad3D(input_resolution, window_size):
    """
    Args:
...
def Pad2D(input_resolution, window_size):
    """
    Args:
    ...
    padding = Pad3D(input_resolution, window_size)
    return padding[: 4]
...
from tensorflow import reshape, transpose as TransposeDimensions
...
def ConstructTensor(shape, value=0.0, dtype=torch.float32):
    return torch.full(shape, value, dtype=dtype)

# TruncatedNormalInit: Initialize the tensor with Truncated Normal distribution
def TruncatedNormalInit(tensor, mean=0.0, std=1.0):
    with torch.no_grad():
...
# TensorSum: a tensor version of numpy.sum
def TensorSum(tensor, axis=None):
    return tensor.sum(dim=axis) 
# LinearSpace: a tensor version of numpy.linspace
def LinearSpace(start, end, steps, dtype=torch.float32):
    return torch.linspace(start, end, steps, dtype=dtype)
...
# LoadModel: Load the model from a file
def LoadModel(model, file_path):
    model.load_state_dict(torch.load(file_path))
    return model

# Backward: Gradient backward to calculate the gradients of each parameter
def Backward(loss):
    loss.backward()

# UpdateModelParametersWithAdam: Use Adam to update parameters
def UpdateModelParametersWithAdam(model, learning_rate, weight_decay=0):
...
def LoadStatic():
    ...
    # Define the filenames for the .pkl files
    pkl_files = {
        'weather_mean': 'upper_air_mean.pkl',
        'weather_std': 'upper_air_std.pkl',
        'weather_surface_mean': 'surface_mean.pkl',
        'weather_surface_std': 'surface_std.pkl'
    }

    # Initialize variables to store the loaded data
    weather_mean = None
    weather_std = None
    weather_surface_mean = None
    weather_surface_std = None

    for var_name, pkl_filename in pkl_files.items():
        ...
        # Convert the data to a NumPy array and save as .npy file
        npy_filename = pkl_filename.replace('.pkl', '.npy')
        npy_path = os.path.join(directory, npy_filename)
        np.save(npy_path, data)
        
        # Load the .npy file into the respective variable
        if var_name == 'weather_mean':
            weather_mean = np.load(npy_path, allow_pickle=True)
        elif var_name == 'weather_std':
            weather_std = np.load(npy_path, allow_pickle=True)
        elif var_name == 'weather_surface_mean':
            weather_surface_mean = np.load(npy_path, allow_pickle=True)
        elif var_name == 'weather_surface_std':
            weather_surface_std = np.load(npy_path, allow_pickle=True)

    return weather_mean, weather_std, weather_surface_mean, weather_surface_std

def LoadConstantMask():
    land_mask = torch.from_numpy(np.load(join("constant_masks", "land_mask.npy")).astype(np.float32))
    soil_type = torch.from_numpy(np.load(join("constant_masks", "soil_type.npy")).astype(np.float32))
    topography = torch.from_numpy(np.load(join("constant_masks", "topography.npy")).astype(np.float32))
...
def convert_nc_to_npy(nc_path, npy_path):
    with Dataset(nc_path, 'r') as nc_file:
        data = {var: nc_file.variables[var][:] for var in nc_file.variables}
        np.save(npy_path, data)
    return npy_path

def LoadData(step, input_data_folder="input_data"):
    # Define paths to the input and target files
    input_nc_path = os.path.join(input_data_folder, "March2023_Upper_00UTC")
    input_surface_nc_path = os.path.join(input_data_folder, "March2023_Surface_00UTC")
    target_nc_path = os.path.join(input_data_folder, "March2023_Upper_06UTC")
    target_surface_nc_path = os.path.join(input_data_folder, "March2023_Surface_06UTC")

    input_npy_path = os.path.join(input_data_folder, f"input_{step}.npy")
    input_surface_npy_path = os.path.join(input_data_folder, f"input_surface_{step}.npy")
    target_npy_path = os.path.join(input_data_folder, f"target_{step}.npy")
    target_surface_npy_path = os.path.join(input_data_folder, f"target_surface_{step}.npy")

    # Convert .nc files to .npy files if they don't already exist
    if not os.path.exists(input_npy_path):
        convert_nc_to_npy(input_nc_path, input_npy_path)
    if not os.path.exists(input_surface_npy_path):
        convert_nc_to_npy(input_surface_nc_path, input_surface_npy_path)
    if not os.path.exists(target_npy_path):
        convert_nc_to_npy(target_nc_path, target_npy_path)
    if not os.path.exists(target_surface_npy_path):
        convert_nc_to_npy(target_surface_nc_path, target_surface_npy_path)

    # Load the data from the .npy files
    input_data = np.load(input_npy_path, allow_pickle=True).item()
    input_surface_data = np.load(input_surface_npy_path, allow_pickle=True).item()
    target_data = np.load(target_npy_path, allow_pickle=True).item()
    target_surface_data = np.load(target_surface_npy_path, allow_pickle=True).item()

    # Extract variables from the dictionaries
    input = input_data['data']
    input_surface = input_surface_data['data']
    target = target_data['data']
    target_surface = target_surface_data['data']

    # Normalize the inputs and targets
    input_mean, input_std = input.mean(), input.std()
    input_surface_mean, input_surface_std = input_surface.mean(), input_surface.std()
    target_mean, target_std = target.mean(), target.std()
    target_surface_mean, target_surface_std = target_surface.mean(), target_surface.std()

    input = normalize(input, input_mean, input_std)
    input_surface = normalize(input_surface, input_surface_mean, input_surface_std)
    target = normalize(target, target_mean, target_std)
    target_surface = normalize(target_surface, target_surface_mean, target_surface_std)

    return input, input_surface, target, target_surface


# UpdateModelParametersWithAdam: Use Adam to update parameters
def UpdateModelParametersWithAdam(model, learning_rate=5e-4, weight_decay=3e-6):
    optimizer = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    optimizer.step()
    optimizer.zero_grad()
...
def SaveModel(model, model_path):
    ...
    path (str): The path where to save the model state dictionary.
    """
    torch.save(model.state_dict(), model_path)

def gen_mask(x, window_size):
    window_size=window_size

    """
    Generate an attention mask for a 3D tensor.
    The mask ensures that attention is only given to adjacent pixels.
    Non-adjacent pixels are masked out by setting the mask value to -1000.

    Parameters:
    x (torch.Tensor): The input tensor of shape [batch_size, channels, depth, height, width].

    Returns:
    torch.Tensor: The attention mask of shape [depth, height, width].
    """
    ...
    mask = torch.zeros((depth, height, width), dtype=torch.float32)
    
    # Set the mask values to -1000 for non-adjacent pixels
...
def no_mask(shape):
    """
    Generate a zero matrix to be used as a mask, meaning no attention masking.

    Parameters:
    shape (tuple): The shape of the mask tensor.

    Returns:
    torch.Tensor: A zero matrix of the specified shape.
    """
...
def Inference(input, input_surface, forecast_range):
  ...
  PanguModel24 = LoadModel("pangu_weather_24.onnx")
  PanguModel6 = LoadModel("pangu_weather_6.onnx")
  PanguModel3 = LoadModel("pangu_weather_3.onnx")
  PanguModel1 = LoadModel("pangu_weather_1.onnx")
  ...
  for i in range(forecast_range):
    ...
    # switch to the 6-hour model if the forecast time is 30 hours, 36 hours, ..., 24*N + 6/12/18 hours
      ...
      global model_path
      model_path="pangu_weather_6.onnx"
    ...
    # switch to the 3-hour model if the forecast time is 3 hours, 9 hours, ..., 6*N + 3 hours
      ...
      global model_path
      model_path="pangu_weather_3.onnx"
...
class EarthSpecificBlock:
  ...
  def forward(self, x, Z, H, W, roll):
    ...
    if roll:
      ...
      mask = gen_mask(x,self.window_size)
    else:
      # e.g., zero matrix when you add mask to attention
      mask = no_mask(x.shape)
...
class EarthAttention3D:
  def __init__(self, dim, heads, dropout_rate, window_size):
    ...
    input_shape = self.current_input_shape
...
if __name__ == "__main__":
   inp, inp_s, t, t_s = LoadData
   Train()
   Inference(inp, inp_s, 6)